...
async function googleLogin(ctx, page) {
  ...
  // Naukri's "Sign in with Google" button
  const googleBtn = page.locator(
    '.socialbtn.google, [class*="socialbtn"][class*="google"], button:has-text("Sign in with Google"), span:has-text("Sign in with Google"), div:has-text("Sign in with Google")'
  ).last();
...
(async () => {
  if (!LOGIN_MODE) {
    const startJitter = Math.floor(Math.random() * 20000) + 5000;
    log(`Background run: applying random start delay of ${(startJitter / 1000).toFixed(1)}s for human behavior...`);
  ...
  try {
    ...
    // Resume headline / Profile summary widget → pencil icon → textarea → save
    const editIcon = page.locator(
      '#lazyResumeHead span.edit.icon, [data-ga-track*="resumeHeadline"] .edit, div:has-text("Profile Summary") > span.new-pencil, span.new-pencil:near(:text("Profile Summary"))'
    );
    await editIcon.first().waitFor({ timeout: 30000 });
    await randomDelay(1000, 2500);
    await editIcon.first().click();

    const textarea = page.locator('#resumeHeadlineTxt, textarea#summary');
    ...
    if (saved !== updated) {
      throw new Error(`save did not stick — server text is "${saved.slice(0, 60)}", expected "${updated.slice(0, 60)}"`);
    }

    log(`OK: profile summary/headline ${current.endsWith('.') ? 'dot removed' : 'dot added'} (verified) → "${updated.slice(0, 60)}"`);
  } catch (err) {
